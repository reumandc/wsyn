...
demonstrations reporoduces an example given in supplementary figure 5 of @Sheppard_16. Data
...
```{r coh_example_dat_driver}
  ...
  x<-matrix(NA,11,length(times)) #the driver
  for (counter in 1:11)
  {
...
```{r coh_example_dat_env}
  times<-0:100
  y<-matrix(NA,11,length(times)) #the driven
...
computational time. The arguments `f0` and `scale.max.input` control details of the
