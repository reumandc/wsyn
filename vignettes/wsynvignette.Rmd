...
Methods comprise set and get methods, and `cohbandtest`, and `plotmag`, `plotrank`, and
`plotphase`.
...
The function `coh` can then be used to compute the coherence between `x` and `y`:
...
@Sheppard_16 and @Sheppard_17. There are several alternative methods for testing the 
...
the default values) quantiles of coherences of surrogate datasets. Coherence is 
significant for timescales close to $10$ years, but not significant for timescales close to
$3$ years, as expected since three values of $x$ are averaged to produce one value of $y$,
so the $3$-year periodicity in $x$ is averaged away.

Typically preferable (see @Sheppard_17) is the `fast` option to `sigmethod`, because 
...
The significance indicated on the above plots is a done on a timescale-by-timescale
...
is implemented in `cohbandtest`:

```{r cohbandtest}
res<-cohbandtest(res,c(8,12))
```

A call to `cohbandtest` computes a
...
```{r cohbandtest_display_bandp}
get_bandp(res)
#***DAN: for some reason this method is not loading
```

The aggregate $p$-value is also displayed on the plot:

```{r, results=FALSE}
plotmag(res)
```

<!--Just do it here with a cursory/black-boxed description of how significance is done, and then intro surrogates in the next
section, and then come back to signifiacnce-->
...
## Alternatives for assessing significance of coherence
