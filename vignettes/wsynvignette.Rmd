...
which are arguments to \code{wt} and several other functions, 
are for constructing the timescales used for 
wavelet analysis. The argument \code{scale.min} is the shortest timescale,
and must be $2$ or greater. Starting from \code{scale.min}, each timescale is 
\code{sigma} times the previous one, up to the first timescale that equals or 
surpasses \code{scale.max.input}. The scalloping of 
wavelet transforms places additional
