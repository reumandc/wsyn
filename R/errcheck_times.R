...
errcheck_times<-function(times,callfunc)
{
  if (!is.numeric(times))
  {
    stop(paste0("Error in ",callfunc,": times must be numeric"))
  }
  if (length(times)<2)
  {
    stop(paste0("Error in ",callfunc,": times must be a vector"))
  }
  d<-diff(times)
  if (!isTRUE(all.equal(rep(d[1],length(d)-1),d[2:length(d)])))
  {
    stop(paste0("Error in ",callfunc,": times must be evenly spaced"))  
  }
  if (d[1]<=0)
  {
    stop(paste0("Error in "callfunc,": times must be increasing"))
  }
}
